...
require_once($CFG->dirroot . '/badges/certificates/edit_form.php');
...
$title = get_string('createcertificate', 'badges');
...
if (($type == CERT_TYPE_COURSE) && ($course = $DB->get_record('course', array('id' => $courseid)))) {
    ...
    $PAGE->set_url('/badges/certificates/new.php', array('type' => $type, 'id' => $course->id));
    ...
    $PAGE->set_url('/badges/certificate/new.php', array('type' => $type));
...
if ($form->is_cancelled()) {
    redirect(new moodle_url('/badges/certificates/index.php', array('type' => $type, 'id' => $courseid)));
    ...
    if (has_capability('moodle/badges:configureelements', $PAGE->context)) {
        redirect(new moodle_url('/badges/certificates/elements.php', array('id' => $newid)));
    }
    redirect(new moodle_url('/badges/certificates/overview.php', array('id' => $newid)));
}
