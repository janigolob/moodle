...
$PAGE->set_url('/badges/certificates/action.php', array('id' => $cert->id));

if ($return !== 0) {
    $returnurl = new moodle_url($return);
} else {
    $returnurl = new moodle_url('/badges/certificates/overview.php', array('id' => $cert->id));
}
...
if ($copy) {
    ...
    if (has_capability('moodle/badges:configurecertificate', $context)) {
        redirect(new moodle_url('/badges/certificates/edit.php', array('id' => $cloneid)));
    }
    redirect(new moodle_url('/badges/certificates/overview.php', array('id' => $cloneid)));
}
...
if ($activate) {
    ...
    $url = new moodle_url('/badges/certificates/action.php', $params);
