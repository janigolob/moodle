...
$hdr = get_string('managebadgecertificates', 'badges');
$returnurl = new moodle_url('/badges/certificates/index.php', $urlparams);
$PAGE->set_url($returnurl);

if ($type == CERT_TYPE_SITE) {
    $title = get_string('sitebadgecertificates', 'badges');
    $PAGE->set_context(context_system::instance());
    $PAGE->set_pagelayout('admin');
    $PAGE->set_heading($title . ': ' . $hdr);
    navigation_node::override_active_url(new moodle_url('/badges/certificates/index.php', array('type' => CERT_TYPE_SITE)), true);
} else {
    require_login($course);
    $coursecontext = context_course::instance($course->id);
    $title = get_string('coursebadgecertificates', 'badges');
    $PAGE->set_context($coursecontext);
    $PAGE->set_pagelayout('course');
    $PAGE->set_heading(format_string($course->fullname, true, array('context' => $coursecontext)) . ': ' . $hdr);
    navigation_node::override_active_url(
        new moodle_url('/badges/certificates/index.php', array('type' => CERT_TYPE_COURSE, 'id' => $course->id))
    );
}
...
if (($delete) && has_capability('moodle/badges:deletecertificate', $PAGE->context)) {
    ...
    if (!$confirm) {
        ...
        echo $output->heading(get_string('delbadgecertificate', 'badges', $cert->name));
        $deletebutton = $output->single_button(
                            new moodle_url($PAGE->url, array('delete' => $cert->id, 'confirm' => 1)),
                            get_string('delconfirmcert', 'badges'));
        echo $output->box(get_string('deletehelpcert', 'badges') . $deletebutton, 'generalbox');
...
echo $OUTPUT->header();
if ($type == CERT_TYPE_SITE) {
    echo $OUTPUT->heading_with_help($PAGE->heading, 'sitebadgecertificates', 'badges');
} else {
...
if ($totalcount) {
    echo $output->heading(get_string('certificatesavailable', 'badges', $totalcount), 4);
    ...
    echo $output->notification(get_string('nobadgecertificates', 'badges'));

    if (has_capability('moodle/badges:createcertificate', $PAGE->context)) {
        echo $OUTPUT->single_button(new moodle_url('new.php', array('type' => $type, 'id' => $courseid)),
            get_string('newbadgecertificate', 'badges'));
    }
}
